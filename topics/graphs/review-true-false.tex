...
\begin{parts}
  ...
  \begin{solution}[0.5in]
    False. Consider the graph $\{(A, B, 1), (B, C, 2)\}$. Even though edge $(B, C)$ is not the lightest edge across the cut $\{B\}, \{A, C\}$, it is necessarily still a part of all MSTs (since this graph is a tree).
  \end{solution}
\end{parts}
